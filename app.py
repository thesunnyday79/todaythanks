...
def make_overlay_frame(img: "Image.Image", scene: dict, product: str, w=1280, h=720) -> "Image.Image":
    """Resize/crop image cleanly to 1280x720 — no text overlays."""
    img    = img.convert("RGB")
    iw, ih = img.size
    scale  = max(w / iw, h / ih)
...
def step4():
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    n_ready  = len(st.session_state.audios)
    lc, rc   = st.columns(2)
    with lc:
        if st.button("← Back", use_container_width=True):
            st.session_state.step = 3; st.rerun()
    with rc:
        if n_ready == 0:
            st.button("🎬 Next → Render Video", use_container_width=True, disabled=True)
            st.markdown("<small style='color:#6a5840'>Generate at least 1 voiceover to continue</small>", unsafe_allow_html=True)
        else:
            if st.button("🎬 Next → Render Video", use_container_width=True):
                st.session_state.step = 5; st.rerun()
            missing = n_total - n_ready
            if missing > 0:
                st.markdown(f"<small style='color:#cc8800'>⚠️ {missing} scene(s) without voiceover will use silence</small>", unsafe_allow_html=True)
            else:
                st.markdown(f"<small style='color:#22c55e'>✅ All {n_total} scenes ready</small>", unsafe_allow_html=True)
...
def step5():
    ...
    n_vo     = len(st.session_state.audios)
    n_sc     = len(scenes)
    vo_color = "#22c55e" if n_vo == n_sc else "#f59e0b"
    vo_note  = f"✅ {n_vo}/{n_sc} scenes voiced" if n_vo == n_sc else f"⚠️ {n_vo}/{n_sc} voiced (others = silence)"
    ...
    🎙️ <span style='color:{vo_color}'>{vo_note}</span> · 🎵 {st.session_state.music}
    </p></div>""", unsafe_allow_html=True)

    if n_vo < n_sc:
        st.markdown(f"""<div class="warn">
        ⚠️ <b>{n_sc - n_vo} scene(s)</b> chưa có voiceover — sẽ render với <b>silence</b>.
        Bạn có thể <a href="#" onclick="return false;" style="color:#fcd34d">quay lại Step 4</a> để record thêm, hoặc tiếp tục render ngay.
        </div>""", unsafe_allow_html=True)
    ...
    # ── Render button
    st.markdown("<br>", unsafe_allow_html=True)
    btn_back, btn_render = st.columns(2)
    with btn_back:
    ...
    with btn_render:
        if st.button("🚀 Render Review Video", use_container_width=True):
            ...
            for i, sc in enumerate(scenes):
                ...
                # ── Audio: use voiceover if available, else silence
                sid = sc["id"]
                has_audio = sid in st.session_state.audios
                if has_audio:
                    ap = os.path.join(tmpdir, f"a{i}.mp3")
                    with open(ap, "wb") as f: f.write(st.session_state.audios[sid])
                    ac  = mpy.AudioFileClip(ap)
                    dur = ac.duration + 0.3
                else:
                    # Default duration for silent scenes: 4 seconds
                    dur = 4.0
                    ac  = None
                ...
                if m and m["type"] == "video":
                    try:
                        ...
                        if vp:
                            ...
                            if ac: vc = vc.set_audio(ac)
                            vc = vc.fadein(0.25).fadeout(0.25)
                            clips.append(vc)
                            clip_made = True
                    except Exception as e:
                ...
                if not clip_made and m and m["type"] == "image":
                    try:
                        ...
                        if ip:
                            ...
                            if ac: ic = ic.set_audio(ac)
                            ic = ic.fadein(0.3).fadeout(0.3)
                            clips.append(ic)
                            clip_made = True
                    except Exception as e:
                        errors.append(f"Image error scene {i+1}: {e}")

                # ── Fallback: solid black frame (no text)
                if not clip_made:
                    try:
                        fb = Image.new("RGB", (1280, 720), (10, 8, 6))
